...
import ct23.xtreme.gameserver.instancemanager.RaidBossSpawnManager.StatusEnum;
...
public class TullyWorkshop extends Quest
{
	...
	public final String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		else if (npcId == DWARVEN_GHOST)
		{
			...
			{
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), Say2.NPC_ALL, npc.getNpcId(), 1800135)); // HA-HA! You were so afraid of death... let me see... If you find me in time... maybe you can... find a way ...
				npc.deleteMe();
				return null;
			}
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			npc.broadcastPacket(new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getNpcId(), 1010631)); // De-activate the alarm.
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		if ((npcId == TULLY) && npc.isInsideRadius(-12557, 273901, -9000, 1000, false, false))
		{
			...
			NpcSay ns = new NpcSay(postMortemSpawn.get(0).getObjectId(), Say2.NPC_SHOUT, postMortemSpawn.get(0).getNpcId(), 1800117); //Detonator initialization- time set for %s minute(s) from now-
		...
		else if ((npcId == TIMETWISTER_GOLEM) && (_countdown != null))
		{
			...
			{
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), Say2.NPC_ALL, npc.getNpcId(), 1000004)); // A fatal error has occurred.
				if (countdownTime > 180000)
				{
					...
					{
						postMortemSpawn.get(0).broadcastPacket(new NpcSay(postMortemSpawn.get(0).getObjectId(), Say2.NPC_SHOUT, postMortemSpawn.get(0).getNpcId(), 1800118)); //Zzzz- city interference error - forward effect created-
					}
				}
			}
			else
			{
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), Say2.NPC_ALL, npc.getNpcId(), 1800113)); // Time rift device activation successful!
				if ((countdownTime > 0) && (countdownTime <= 420000))
				{
					countdownTime += 180000;
					if ((postMortemSpawn != null) && (postMortemSpawn.size() > 0) && (postMortemSpawn.get(0) != null) && (postMortemSpawn.get(0).getNpcId() == INGENIOUS_CONTRAPTION))
					{
						postMortemSpawn.get(0).broadcastPacket(new NpcSay(postMortemSpawn.get(0).getObjectId(), Say2.NPC_SHOUT, postMortemSpawn.get(0).getNpcId(), 1800119)); //Zzzz- city interference error - recurrence effect created-
					}
				}
			}
		}
		...
		else if ((npcId >= SERVANT_FIRST) && (npcId <= SERVANT_LAST))
		{
			...
			{
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getNpcId(), 1800130)); //I failed... Please forgive me, Darion...
			}
			else
			{
				NpcSay ns = new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getNpcId(), 1800131); //%s, I'll be back... don't get comfortable...
	...
	{
		if ((npc.getNpcId() == TULLY) && npc.isInsideRadius(-12557, 273901, -9000, 1000, true, false))
		{
	...
	public String onSpellFinished(L2Npc npc, L2PcInstance player, L2Skill skill)
	{
		...
		{
			final NpcSay ns = new NpcSay(npc.getObjectId(), Say2.NPC_ALL, npc.getNpcId(), 1800129); //%s, thank you... for giving me your life...
			ns.addStringParameter(player.getName());
			npc.broadcastPacket(ns);
			
			final int dmg = (int) (player.getCurrentHp() / (npc.getNpcId() - 22404));
			player.reduceCurrentHp(dmg, null, null);
			npc.setCurrentHp((npc.getCurrentHp() + 10) - (npc.getNpcId() - 22404));
		}
		return null;
	}
		...
		public void run()
		{
			...
			if (countdownTime > 60000)
			{
				if ((countdownTime % 60000) == 0)
				{
					...
					{
						NpcSay ns = new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getNpcId(), 1010643); // %s minute(s) are remaining.
						ns.addStringParameter(Integer.toString((countdownTime / 60000)));
						npc.broadcastPacket(ns);
					}
				}
			...
			else if (countdownTime <= 0)
			{
				...
				for (L2Npc spawnedNpc : postMortemSpawn)
				{
					if ((spawnedNpc != null) && ((spawnedNpc.getNpcId() == INGENIOUS_CONTRAPTION) || (spawnedNpc.getNpcId() == TIMETWISTER_GOLEM)))
					{
						spawnedNpc.deleteMe();
					}
				}
			...
			else
			{
				if ((npc != null) && (npc.getNpcId() == INGENIOUS_CONTRAPTION))
				{
					final NpcSay ns = new NpcSay(npc.getObjectId(), Say2.NPC_SHOUT, npc.getNpcId(), 1800079); // %s second(s) remaining.
