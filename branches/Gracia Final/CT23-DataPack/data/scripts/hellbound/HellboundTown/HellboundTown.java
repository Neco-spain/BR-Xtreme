...
import ct23.xtreme.gameserver.model.actor.instance.L2QuestGuardInstance;
...
public class HellboundTown extends Quest
{
			...
			if (npc.getNpcId() == DOWNTOWN_NATIVE)
			{
				...
				else if (event.equalsIgnoreCase("break_chains"))
				{
					...
					else
					{
						...
						if (world.spawnedAmaskari != null && !world.spawnedAmaskari.isDead() && getRandom(1000) < 25 &&
	...
	private boolean checkTeleport(L2PcInstance player)
	{
		...
		for (L2PcInstance partyMember : party.getPartyMembers())
		{
			...
			{
				final SystemMessage sm = new SystemMessage(SystemMessageId.ALREADY_ENTERED_ANOTHER_INSTANCE_CANT_ENTER);
				sm.addPcName(partyMember);
				party.broadcastToPartyMembers(sm);
				return false;
			}
