...
import ct23.xtreme.gameserver.model.L2Territory;
...
/**
 * TODO: reworked by @Browser 
 * Refactor Script BRXtreme Freya Project, L2jServer and L2Emu Teanm
...
public class SeedOfDestruction extends Quest
{
	...
	private void spawnFlaggedNPCs(SODWorld world, int flag)
	{
	...
	protected boolean spawnState(SODWorld world)
	{
		if (world.lock.tryLock())
		{
			try
	...
	protected void spawn(SODWorld world, int npcId, int x, int y, int z, int h, boolean addToKillTable)
	{
		// traps
	...
	public String onTalk (L2Npc npc, L2PcInstance player)
	{
		...
		{
			InstanceWorld world = InstanceManager.getInstance().getPlayerWorld(player);
			if (GraciaSeedsManager.getInstance().getSoDState() == 1 || (world != null && world instanceof SODWorld))
