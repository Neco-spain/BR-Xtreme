...
public class CrystalCaverns extends Quest {
	...
	private class CCWorld extends InstanceWorld {
		...
		public int[] roomsStatus = { 0, 0, 0, 0 }; // 0: not spawned, 1:
													// spawned, 2: cleared
	...
	private static final int[] CGMOBS = { 22311, 22312, 22313, 22314, 22315,
			22316, 22317 };
	private static final int[] SPAWN = { 60000, 120000, 90000, 60000, 50000,
			40000 }; // Kechi Hencmans spawn times
	private static final int[] MOBLIST = { 22279, 22280, 22281, 22282, 22283,
			22285, 22286, 22287, 22288, 22289, 22293, 22294, 22295, 22296,
			22297, 22305, 22306, 22307, 22416, 22418, 22419, 22420 };
	...
	// Baylor alarm spawns
	private final static int[][] ALARMSPAWN = { { 153572, 141277, -12738 },
			{ 153572, 142852, -12738 }, { 154358, 142075, -12738 },
			{ 152788, 142075, -12738 } };
	...
	// Hall spawns
	private static int[][] SPAWNS = { { 141842, 152556, -11814, 50449 },
	...
	// {22308, 145093, 152502, -12165, 31841},{22308, 146158, 152776, -12165,
	// 30810},
	...
	private static int[][] STEAM2_SPAWNS = {
			...
			{ 22306, 147743, 152846, -12165, 64147 },// {22308, 147849, 152854,
														// -12165, 60534},
			// {22308, 147754, 152908, -12141, 59827},{22308, 148194, 152681,
			// -12165, 63620},
			// {22308, 147767, 152939, -12133, 63381},{22309, 147737, 152671,
			// -12165, 65320},
	...
	/*
	 * {22308, 150794, 152455, -12165, 31613},{22308, 150665, 152383, -12165,
	 * 32767}, {22308, 151697, 152621, -12167, 31423},{22309, 151061, 152581,
	 * -12165, 6228}, {22309, 150653, 152253, -12132, 31343},{22309, 150628,
	 * 152431, -12165, 33022}, {22309, 151620, 152487, -12165, 30114},{22309,
	 * 151672, 152544, -12165, 31846}, {22309, 150488, 152350, -12165,
	 * 29072},{22310, 151139, 152238, -12132, 1069}
	 */
	};

	private static int[][] STEAM4_SPAWNS = {// {22308, 151707, 150199, -12165,
											// 32859},{22308, 152091, 150140,
											// -12165, 32938},
			// {22308, 149757, 150204, -12138, 65331},{22308, 149950, 150307,
			// -12132, 62437},
			// {22308, 149901, 150322, -12132, 62136},{22309, 150071, 150173,
			// -12165, 64943},
			...
			{ 22420, 150744, 150006, -12165, 63 } // ,{22417, 149782, 150188,
													// -12151, 64001}
	};
	...
	protected void openDoor(int doorId, int instanceId) {
		for (L2DoorInstance door : InstanceManager.getInstance()
				.getInstance(instanceId).getDoors())
	...
	private boolean checkConditions(L2PcInstance player) {
		...
		for (L2PcInstance partyMember : party.getPartyMembers()) {
			...
			L2ItemInstance item = partyMember.getInventory().getItemByItemId(
					CONT_CRYSTAL);
			if (item == null) {
			...
			Long reentertime = InstanceManager.getInstance().getInstanceTime(
					partyMember.getObjectId(), INSTANCEID);
	...
	private boolean checkBaylorConditions(L2PcInstance player) {
		...
		if (party == null) {
			player.sendPacket(new SystemMessage(
					SystemMessageId.NOT_IN_PARTY_CANT_ENTER));
			return false;
		}
		if (party.getLeader() != player) {
			player.sendPacket(new SystemMessage(
					SystemMessageId.ONLY_PARTY_LEADER_CAN_ENTER));
			return false;
		}
		for (L2PcInstance partyMember : party.getPartyMembers()) {
			L2ItemInstance item1 = partyMember.getInventory().getItemByItemId(
					BOSS_CRYSTAL_1);
			L2ItemInstance item2 = partyMember.getInventory().getItemByItemId(
					BOSS_CRYSTAL_2);
			L2ItemInstance item3 = partyMember.getInventory().getItemByItemId(
					BOSS_CRYSTAL_3);
	...
	private void Throw(L2Character effector, L2Character effected) {
		...
		if (Config.GEODATA > 0) {
			Location destiny = GeoData.getInstance().moveCheck(effected.getX(),
					effected.getY(), effected.getZ(), _x, _y, _z,
					effected.getInstanceId());
			_x = destiny.getX();
			_y = destiny.getY();
		}
		effected.broadcastPacket(new FlyToLocation(effected, _x, _y, _z,
				FlyType.THROW_UP));
	...
	protected int enterInstance(L2PcInstance player, String template,
		...
		// New instance
			...
			_log.info("Crystal Caverns started " + template + " Instance: "
					+ instanceId + " created by player: " + player.getName());
			...
			if (player.getParty() == null) {
				...
				InstanceManager.getInstance().setInstanceTime(
						...
						((System.currentTimeMillis() + INSTANCEPENALTY)));
				teleportplayer(player, teleto);
	...
	protected void runOracle(CCWorld world) {
		...
		world.oracle.add(addSpawn(ORACLE_GUIDE_1, 143172, 148894, -11975, 0,
	...
	protected void runHall(CCWorld world) {
		...
		for (int[] spawn : SPAWNS) {
			L2Npc mob = addSpawn(CGMOBS[Rnd.get(CGMOBS.length)], spawn[0],
					spawn[1], spawn[2], spawn[3], false, 0, false,
					world.instanceId);
	...
	protected void runFirst(CCWorld world) {
		...
		world.keyKeepers.add(addSpawn(GK1, 148206, 149486, -12140, 32308,
				false, 0, false, world.instanceId));
		world.keyKeepers.add(addSpawn(GK2, 148203, 151093, -12140, 31100,
				false, 0, false, world.instanceId));

		for (int[] spawn : FIRST_SPAWNS) {
			addSpawn(spawn[0], spawn[1], spawn[2], spawn[3], spawn[4], false,
	...
	protected void runEmeraldSquare(CCWorld world) {
		...
		for (int[] spawn : EMERALD_SPAWNS) {
			L2Npc mob = addSpawn(spawn[0], spawn[1], spawn[2], spawn[3],
					spawn[4], false, 0, false, world.instanceId);
	...
	protected void runEmeraldRooms(CCWorld world, int[][] spawnList, int room) {
		...
		if (room == 1) // spawn Lahm
			addSpawn(32359, 142110, 139896, -11888, 8033, false, 0, false,
					world.instanceId);
	...
	protected void runDarnel(CCWorld world) {
		...
		addSpawn(DARNEL, 152759, 145949, -12588, 21592, false, 0, false,
				world.instanceId);
		// TODO: missing traps
	...
	protected void runSteamOracles(CCWorld world, int[][] oracleOrder) {
		...
		for (int[] oracle : oracleOrder) {
			world.oracles
					.put(addSpawn(oracle[0], oracle[1], oracle[2], oracle[3],
							oracle[4], false, 0, false, world.instanceId), null);
		}
	}
	...
	/*
	 * protected void runBaylorRoom(CCWorld world) { world.status = 30;
	 * 
	 * addSpawn(29101,152758,143479,-12706,52961,false,0,false,world.instanceId,0
	 * );//up power
	 * addSpawn(29101,151951,142078,-12706,65203,false,0,false,world
	 * .instanceId,0);//up power
	 * addSpawn(29101,154396,140667,-12706,22197,false,
	 * 0,false,world.instanceId,0);//up power
	 * addSpawn(29102,152162,141249,-12706
	 * ,5511,false,0,false,world.instanceId,0);//down power
	 * addSpawn(29102,153571
	 * ,140458,-12706,16699,false,0,false,world.instanceId,0);//down power
	 * addSpawn
	 * (29102,154976,141265,-12706,26908,false,0,false,world.instanceId,0
	 * );//down power
	 * addSpawn(29102,155203,142071,-12706,31560,false,0,false,world
	 * .instanceId,0);//down power
	 * addSpawn(29102,154380,143468,-12708,43943,false
	 * ,0,false,world.instanceId,0);//down power
	 * addSpawn(32271,153573,142069,-9722,11175,false,0,false,world.instanceId);
	 * world.Baylor =
	 * addSpawn(BAYLOR,153557,142089,-12735,11175,false,0,false,world
	 * .instanceId,0);
	 * 
	 * }
	 */
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player) {
		if (npc.getNpcId() == ORACLE_GUIDE_1) {
			InstanceWorld tmpworld = InstanceManager.getInstance().getWorld(
					npc.getInstanceId());
	...
	public String onSkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill,
		...
		if (npc.getNpcId() >= 32275 && npc.getNpcId() <= 32277
				&& skill.getId() != 2360 && skill.getId() != 2369) {
			InstanceWorld tmpworld = InstanceManager.getInstance().getWorld(
					npc.getInstanceId());
		...
		} else if (npc.isInvul() && npc.getNpcId() == BAYLOR
				&& skill.getId() == 2360 && caster != null) {
			...
			if (tmpworld instanceof CCWorld) {
				...
				if ((world._dragonClawStart + DRAGONCLAWTIME) <= System
						.currentTimeMillis() || world._dragonClawNeed <= 0) {
				...
				if (world._dragonClawNeed == 0) {
					...
					npc.broadcastPacket(new MagicSkillUse(npc, npc, 5480, 1,
							4000, 0));
					if (world._raidStatus == 3)
						world._raidStatus++;
				}
			}
		} else if (npc.isInvul() && npc.getNpcId() == TEARS
				&& skill.getId() == 2369 && caster != null) {
			InstanceWorld tmpworld = InstanceManager.getInstance().getWorld(
					npc.getInstanceId());
			if (tmpworld instanceof CCWorld) {
				CCWorld world = (CCWorld) tmpworld;
				if (caster.getParty() == null) {
					return super.onSkillSee(npc, caster, skill, targets, isPet);
				} else if ((world.dragonScaleStart + DRAGONSCALETIME) <= System
						.currentTimeMillis() || world.dragonScaleNeed <= 0) {
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage,
			boolean isPet, L2Skill skill) {
		if (npc.getNpcId() == TEARS) {
			...
			if (tmpworld instanceof CCWorld) {
				...
				if (world.status != 4 && attacker != null) {
					// Lucky cheater, the code only kicks his/her ass out of the
					// dungeon
					...
					world.allowed.remove(world.allowed.indexOf(attacker
							.getObjectId()));
					...
					boolean notAOE = true;
					if (skill != null
							&& (skill.getTargetType() == SkillTargetType.TARGET_AREA
									...
									|| skill.getTargetType() == SkillTargetType.TARGET_BEHIND_AURA || skill
				...
				if (nowHp < maxHp * 0.4 && rand < 5) {
					...
					for (int i = 0; i < 10; i++) {
						L2Npc copy = addSpawn(TEARS_COPY, npc.getX(),
								npc.getY(), npc.getZ(), 0, false, 0, false,
								attacker.getInstanceId());
						copy.setRunning();
						((L2Attackable) copy).addDamageHate(target, 0, 99999);
						copy.getAI().setIntention(
								CtrlIntention.AI_INTENTION_ATTACK, target);
		...
		if (tmpworld instanceof CCWorld) {
			...
			if (event.equalsIgnoreCase("TeleportOut")) {
				...
				if (player.getInstanceId() == world.instanceId) {
					...
					SkillTable.getInstance().getInfo(5239, 1)
				...
				InstanceManager.getInstance().getInstance(world.instanceId)
						.removeNpcs();
				...
				world.guards.add(addSpawn(KECHIGUARD, 153622, 149699, -12131,
						56890, false, 0, false, world.instanceId));
				world.guards.add(addSpawn(KECHIGUARD, 153609, 149622, -12131,
						64023, false, 0, false, world.instanceId));
				world.guards.add(addSpawn(KECHIGUARD, 153606, 149428, -12131,
						64541, false, 0, false, world.instanceId));
				world.guards.add(addSpawn(KECHIGUARD, 153601, 149534, -12131,
						64901, false, 0, false, world.instanceId));
				world.guards.add(addSpawn(KECHIGUARD, 153620, 149354, -12131,
						1164, false, 0, false, world.instanceId));
				world.guards.add(addSpawn(KECHIGUARD, 153637, 149776, -12131,
						61733, false, 0, false, world.instanceId));
				world.guards.add(addSpawn(KECHIGUARD, 153638, 149292, -12131,
						64071, false, 0, false, world.instanceId));
				world.guards.add(addSpawn(KECHIGUARD, 153647, 149857, -12131,
						59402, false, 0, false, world.instanceId));
				world.guards.add(addSpawn(KECHIGUARD, 153661, 149227, -12131,
						65275, false, 0, false, world.instanceId));
				if (world.kechisHenchmanSpawn <= 5)
					startQuestTimer("spawnGuards",
							SPAWN[world.kechisHenchmanSpawn], npc, null);
				else
				...
				addSpawn(32271, 153572, 142075, -9728, 10800, false, 0, false,
						world.instanceId);
				addSpawn((Rnd.get(10) < 5 ? 29116 : 29117), npc.getX(),
						npc.getY(), npc.getZ(), npc.getHeading(), false, 0,
						false, world.instanceId); // Baylor's Chest
				addSpawn(ORACLE_GUIDE_4, 153572, 142075, -12738, 10800, false,
						0, false, world.instanceId);
				...
				npc.broadcastPacket(new SpecialCamera(npc.getObjectId(), 500,
						-45, 170, 5000, 9000));
			} else if (event.equalsIgnoreCase("baylorEffect1")) {
				npc.broadcastPacket(new SpecialCamera(npc.getObjectId(), 300,
						0, 120, 2000, 5000));
				npc.broadcastPacket(new SocialAction(npc.getObjectId(), 3));
				startQuestTimer("baylorEffect2", 4000, npc, null);
			} else if (event.equalsIgnoreCase("baylorEffect2")) {
				npc.broadcastPacket(new SpecialCamera(npc.getObjectId(), 747,
						0, 160, 2000, 3000));
				npc.broadcastPacket(new MagicSkillUse(npc, npc, 5402, 1, 2000,
						0));
				...
				for (int i = 0; i < 10; i++) {
					...
					L2Npc mob = addSpawn(29104, 153571 + x, 142075 + y, -12737,
							0, false, 0, false, world.instanceId);
				...
				if (world._alarm == null) {
					...
					world._alarm = addSpawn(ALARMID, spawnLoc[0], spawnLoc[1],
							spawnLoc[2], 10800, false, 0, false,
							world.instanceId);
					world._alarm.disableCoreAI(true);
					world._alarm.setIsImmobilized(true);
					world._alarm
							.broadcastPacket(new CreatureSay(
				...
				if (world._baylor == null) {
					...
					if (nowHp < maxHp * 0.2 && world._raidStatus < 3
							&& npc.getFirstEffect(5224) == null
							&& npc.getFirstEffect(5225) == null) {
						if (nowHp < maxHp * 0.15 && world._raidStatus == 2) {
							npc.doCast(SkillTable.getInstance()
									.getInfo(5225, 1));
							npc.broadcastPacket(new CreatureSay(npc
									.getObjectId(), 1, npc.getName(),
							...
							npc.broadcastPacket(new CreatureSay(npc
									.getObjectId(), 1, npc.getName(),
									"Demons King Beleth, give me power! Aaaaa!!!"));
							startQuestTimer("baylor_remove_invul", 30000,
									world._baylor, null);
						}
					} else if (nowHp < maxHp * 0.3 && rand > 50
							&& npc.getFirstEffect(5225) == null
							&& npc.getFirstEffect(5224) == null) {
						npc.doCast(SkillTable.getInstance().getInfo(5224, 1));
					} else if (rand < 33) {
						npc.setTarget(world._raiders.get(Rnd.get(world._raiders
								.size())));
				...
				world._baylor = addSpawn(29099, 153572, 142075, -12738, 10800,
						false, 0, false, world.instanceId);
				world._baylor.setIsParalyzed(true);
				world._camera = addSpawn(29120, 153273, 141400, -12738, 10800,
						false, 0, false, world.instanceId);
				world._camera.broadcastPacket(new SpecialCamera(world._camera
						.getObjectId(), 700, -45, 160, 500, 15200));
				...
				for (L2Object object : L2World.getInstance().getVisibleObjects(
						npc, 300)) {
					if (object instanceof L2ItemInstance
							&& ((L2ItemInstance) object).getItemId() == CRYSTALFOOD)
						crystals.add(object);
				}
				int minDist = 300000;
				...
				if (minDist != 300000)
					...
					if (Rnd.get(100) < 5)
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(),
								1, npc.getName(), "ah ... I am so hungry ..."));
					startQuestTimer("autoFood", 2000, npc, null);
				}
				return "";
			} else if (!world.crystalGolems.containsKey(npc)
					|| world.crystalGolems.get(npc).isAtDestination)
				return "";
				...
				if (cryGolem.foodItem == null || !cryGolem.foodItem.isVisible()) {
					...
					L2World.getInstance().removeVisibleObject(
							cryGolem.foodItem,
							cryGolem.foodItem.getWorldRegion());
					...
					if (d1 < 10000 || d2 < 10000) {
						npc.broadcastPacket(new MagicSkillUse(npc, npc, 5441,
								1, 1, 0));
				...
				CrystalGolem cryGolem = world.crystalGolems.get(npc);
				L2CharPosition newpos = new L2CharPosition(
						cryGolem.foodItem.getX(), cryGolem.foodItem.getY(),
						cryGolem.foodItem.getZ(), 0);
				cryGolem.oldpos = new L2CharPosition(npc.getX(), npc.getY(),
	...
	private void giveRewards(L2PcInstance player, int instanceId, int bossCry,
			boolean isBaylor) {
		L2Party party = player.getParty();
		if (party != null) {
			for (L2PcInstance partyMember : party.getPartyMembers())
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isPet) {
		...
		if (tmpworld instanceof CCWorld) {
			...
			if (world.status == 2 && world.npcList1.containsKey(npc)) {
				...
				for (boolean isDead : world.npcList1.values())
					if (!isDead)
						return "";
				world.status = 3;
				world.tears = addSpawn(TEARS, 144298, 154420, -11854, 32767,
						false, 0, false, world.instanceId); // Tears
				CrystalGolem crygolem1 = new CrystalGolem();
				CrystalGolem crygolem2 = new CrystalGolem();
				world.crystalGolems.put(
						addSpawn(CRYSTAL_GOLEM, 140547, 151670, -11813, 32767,
								false, 0, false, world.instanceId), crygolem1);
				world.crystalGolems.put(
						addSpawn(CRYSTAL_GOLEM, 141941, 151684, -11813, 63371,
								false, 0, false, world.instanceId), crygolem2);
				...
				InstanceManager.getInstance().getInstance(world.instanceId)
						.setDuration(300000);
				addSpawn(32280, 144312, 154420, -11855, 0, false, 0, false,
						world.instanceId);
					...
					if (party != null)
						for (L2PcInstance partyMember : party.getPartyMembers()) {
							if (partyMember.getInstanceId() == world.instanceId) {
								SkillTable.getInstance().getInfo(5239, 1)
										.getEffects(partyMember, partyMember);
								startQuestTimer("Timer2", 300000, npc,
										partyMember);
							}
						}
					else {
				...
				if (checkKillProgress(0, npc, world)) {
					world.status = 4;
					addSpawn(TOURMALINE, 148202, 144791, -12235, 0, false, 0,
				...
				if (npc.getNpcId() == TOURMALINE) {
					world.status = 5;
					addSpawn(TEROD, 147777, 146780, -12281, 0, false, 0, false,
							world.instanceId);
				}
			} else if (world.status == 5) {
				if (npc.getNpcId() == TEROD) {
					world.status = 6;
					addSpawn(TOURMALINE, 143694, 142659, -11882, 0, false, 0,
							false, world.instanceId);
				}
			} else if (world.status == 6) {
				if (npc.getNpcId() == TOURMALINE) {
					world.status = 7;
					addSpawn(DOLPH, 142054, 143288, -11825, 0, false, 0, false,
							world.instanceId);
				}
				...
				for (int i = 0; i < 4; i++) {
					if (world.roomsStatus[i] == 1
							&& checkKillProgress(i + 1, npc, world)) {
				...
				if (checkKillProgress(0, npc, world)) {
					...
					case 25:
						...
						L2Party party = player.getParty();
						if (party != null)
							for (L2PcInstance partyMember : party
									.getPartyMembers())
						...
						L2Npc kechi = addSpawn(KECHI, 154069, 149525, -12158,
								51165, false, 0, false, world.instanceId);
						startQuestTimer("checkKechiAttack", 1000, kechi, null);
						return "";
					default:
						_log.warning("CrystalCavern-SteamCorridor: status "
								+ world.status
			...
			} else if ((world.status == 9 && npc.getNpcId() == DARNEL)
					|| (world.status == 26 && npc.getNpcId() == KECHI)) {
				InstanceManager.getInstance().getInstance(world.instanceId)
						.setDuration(300000);
				int bossCry;
				if (npc.getNpcId() == KECHI) {
					...
					cancelQuestTimers("spawnGuards");
					addSpawn(32280, 154077, 149527, -12159, 0, false, 0, false,
							world.instanceId);
				} else if (npc.getNpcId() == DARNEL) {
					bossCry = BOSS_CRYSTAL_1;
					addSpawn(32280, 152761, 145950, -12588, 0, false, 0, false,
							world.instanceId);
				} else {
					// something is wrong
					return "";
			...
			if (npc.getNpcId() == ALARMID) {
				...
				world._alarm = null;
				if (world._baylor.getMaxHp() * 0.3 < world._baylor.getStatus()
						.getCurrentHp())
					startQuestTimer("baylor_alarm", 40000, world._baylor, null);
			} else if (npc.getNpcId() == BAYLOR) {
				world._baylor = null;
				npc.broadcastPacket(new PlaySound(1, "BS01_D", 1, npc
						.getObjectId(), npc.getX(), npc.getY(), npc.getZ()));
				Instance baylorInstance = InstanceManager.getInstance()
						.getInstance(npc.getInstanceId());
	...
	public String onTalk(L2Npc npc, L2PcInstance player) {
		...
		if (tmpworld instanceof CCWorld) {
			...
			if (npcId == CRYSTAL_GOLEM) {
					...
					&& world.OracleTriggered[npc.getNpcId() - 32275]) {
				...
				case 32275:
					...
					if (party != null)
						for (L2PcInstance partyMember : party.getPartyMembers()) {
							if (partyMember.getInstanceId() == world.instanceId) {
								partyMember.stopSkillEffects(5239);
								SkillTable.getInstance().getInfo(5239, 2)
										.getEffects(partyMember, partyMember);
								startQuestTimer("Timer3", 600000, npc,
										partyMember);
							}
						}
					else {
						...
						for (L2PcInstance partyMember : party.getPartyMembers()) {
							if (partyMember.getInstanceId() == world.instanceId) {
								...
								SkillTable.getInstance().getInfo(5239, 4)
										.getEffects(partyMember, partyMember);
								startQuestTimer("Timer4", 1200000, npc,
										partyMember);
							}
						}
					else {
				...
				case 32277:
					...
					if (party != null)
						for (L2PcInstance partyMember : party.getPartyMembers()) {
							if (partyMember.getInstanceId() == world.instanceId) {
								partyMember.stopSkillEffects(5239);
								SkillTable.getInstance().getInfo(5239, 3)
										.getEffects(partyMember, partyMember);
								startQuestTimer("Timer5", 900000, npc,
										partyMember);
							}
						}
					else {
				...
				if (doTeleport) {
					if (!checkOracleConditions(player))
						...
						for (L2PcInstance partyMember : party.getPartyMembers()) {
							partyMember.destroyItemByItemId("Quest", RED_CORAL,
									1, player, true);
							teleportplayer(partyMember, teleto);
						}
					else {
				...
				if (checkBaylorConditions(player)) {
					...
					if (party == null)
						...
						for (L2PcInstance partyMember : party.getPartyMembers()) {
							int rnd = Rnd.get(100);
							partyMember.destroyItemByItemId("Quest",
									(rnd < 33 ? BOSS_CRYSTAL_1
											: (rnd < 67 ? BOSS_CRYSTAL_2
													: BOSS_CRYSTAL_3)), 1,
									partyMember, true);
				...
				for (L2PcInstance p : world._raiders) {
					...
					int y = (int) (radius * Math.sin(i++ * 2 * Math.PI
							/ members));
		...
		if (character instanceof L2PcInstance) {
			...
			if (tmpworld instanceof CCWorld) {
				...
				if (world.status == 8) {
					...
					for (L2DoorInstance door : InstanceManager.getInstance()
							.getInstance(world.instanceId).getDoors())
						if (door.getDoorId() == (room + 24220000)) {
							if (door.getOpen())
								return "";
							else {
								QuestState st = ((L2PcInstance) character)
										.getQuestState(qn);
			...
			if (tmpworld instanceof CCWorld) {
				...
				if (world.status == 8) {
					...
					for (L2DoorInstance door : InstanceManager.getInstance()
						...
						if (door.getDoorId() == doorId) {
							if (door.getOpen()
									&& world.openedDoors.get(door) == character) {
								door.closeMe();
	...
	public CrystalCaverns(int questId, String name, String descr) {
		...
		for (int npc : Talk)
			addTalkId(npc);
		int[] firstTalk = { 32274, 32275, 32276, 32277, ORACLE_GUIDE_1,
				ORACLE_GUIDE_2 };
