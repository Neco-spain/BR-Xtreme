...
## Bug reports: BR Xtreme Web Forum       ##
############################################
trap finish 2
...
echo -ne "\nPlease enter MySQL Login Server database name (default ct23db): "
read LSDB
if [ -z "$LSDB" ]; then
  LSDB="ct23db"
fi
...
echo -ne "\nPlease enter MySQL Community Server database name (default ct23cb): "
read CBDB
if [ -z "$CBDB" ]; then
  CBDB="ct23cb"
fi
...
# BR Xtreme  mods that needed extra tables to work properly, should be 
