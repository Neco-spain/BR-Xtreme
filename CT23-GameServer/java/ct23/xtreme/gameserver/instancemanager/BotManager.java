...
public class BotManager
{
    ...
    public synchronized void reportBot(L2PcInstance reported, L2PcInstance reporter)
    {
        ...
        SystemMessage sm = new SystemMessage(SystemMessageId.C1_WAS_REPORTED_AS_BOT);
    ...
    public boolean validateBot(L2PcInstance reported, L2PcInstance reporter)
    {
        ...
        {
            reporter.sendPacket(new SystemMessage(SystemMessageId.TARGET_NOT_REPORT_CANNOT_REPORT_PEACE_PVP_ZONE_OR_OLYMPIAD_OR_CLAN_WAR_ENEMY));
            return false;
        }
        ...
        if (reported.getClan() != null && reporter.getClan() != null)
        {
            ...
            {
                reporter.sendPacket(new SystemMessage(SystemMessageId.YOU_CANNOT_REPORT_CLAN_WAR_ENEMY));
                return false;
            }
        }
        ...
        {
            reporter.sendPacket(new SystemMessage(SystemMessageId.YOU_CANNOT_REPORT_CHAR_WHO_ACQUIRED_XP));
            return false;
        }
        ...
        if (_reportedCount.containsKey(reporter))
        {
            for (L2PcInstance p : _reportedCount.get(reporter))
            {
                if (reported == p)
                {
                    reporter.sendPacket(new SystemMessage(SystemMessageId.C1_WAS_REPORTED_AS_BOT));
                    return false;
                }
            }
        }
    ...
    public synchronized boolean validateReport(L2PcInstance reporter)
    {
        ...
        {
            reporter.sendPacket(new SystemMessage(SystemMessageId.CANNOT_REPORT_TARGET_ALREDY_REPORTED_BY_CLAN_ALLY_MEMBER_OR_SAME_IP));
            return false;
        }
        ...
        {
            reporter.sendPacket(new SystemMessage(SystemMessageId.CANNOT_REPORT_ALREDY_REPORED_FROM_THIS_ACCOUNT));
            return false;
        }
    ...
    private void restorePlayerBotPunishment(L2PcInstance activeChar)
    {
        ...
        if (!punish.isEmpty() && BotPunish.PunishType.valueOf(punish) != null)
        {
            if (delay < 0)
            {
                BotPunish.PunishType p = BotPunish.PunishType.valueOf(punish);
